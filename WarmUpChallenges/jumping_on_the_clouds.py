...
plus 1 or 2. She must avoid the thunderheads. Determine the minimum number of jumps
it will take Emma to jump from her starting position to the last cloud. It is
...
def jumping_on_clouds(c):
    no_of_jumps, cur_pos, pre_pos = (0, 0, 0)

    for i in range(len(c)):
        try:
            cur_pos += TWO_JUMPS if c[cur_pos + TWO_JUMPS] == 0 else (ONE_JUMP if c[cur_pos + ONE_JUMP] == 0 else 0)
            if pre_pos != cur_pos:
                no_of_jumps += 1
                pre_pos = cur_pos
        except IndexError:
            no_of_jumps += ONE_JUMP if cur_pos < len(c)-1 else 0
            break
...
if __name__ == '__main__':
    n = int(input("Enter the total no of clouds: "))

    c = list(map(int, input("Enter the space separated binary "
                            "integers: ").rstrip().split()))
