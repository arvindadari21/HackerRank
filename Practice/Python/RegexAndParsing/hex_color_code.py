...
You are given N lines of CSS code. Your task is to print all valid Hex Color
Codes, in order of their occurrence from top to bottom.
...
Explanation:
    #BED and #Cab satisfy the Hex Color Code criteria, but they are used as
    selectors and not as color codes in the given CSS.
...
for _ in range(int(input())):
    line = input().strip()
    for color in re.finditer(r'(?<=[:\s])+(#[\da-fA-F]{3,6})', line):
        print(color.group())
