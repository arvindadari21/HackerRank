...
* The username starts with an English alphabetical character, and any subsequent
  characters consist of one or more of the following: alphanumeric
  characters, -,., and _.
...
Input Format:
    * The first line contains a single integer, n, denoting the number of email
      address.
...
Output Format:
    ...
    email addresses only. Each pair must be printed on a new line in the
    following format:
        name <user@email.com>

    You must print each valid email address in the same order as it was
    received as input.
...
for _ in range(int(input())):
    ...
    if re.match(r'^<[^\.\-\_]{1}[a-z0-9\.\-\_]+@[a-z]+\.[a-z]{1,3}>$', email,
                re.IGNORECASE):
        print(name, email)
