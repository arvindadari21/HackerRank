...
here(https://docs.python.org/2/library/itertools.html).
...
Explanation:
    All possible unordered tuples of length 2 comprising of indices from 1 to 4
    are:
...
if __name__ == '__main__':
    len_list = int(input())
    list_elements = input().strip().split()
    k = int(input())

    count = 0
    no_of_comb = 0
    possible_combinations = combinations(list_elements, k)

    for one in possible_combinations:
        count += 1 if 'a' in one else 0
        no_of_comb += 1

    print('{:02f}'.format(count / no_of_comb))
