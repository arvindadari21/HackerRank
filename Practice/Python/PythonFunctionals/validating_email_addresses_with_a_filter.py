...
You are given an integer N followed by N email addresses. Your task is to print
...
Concept:
    A filter takes a function returning True or False and applies it to a
    sequence, returning a list of only those members of the sequence where the
    function returned True. A Lambda function can be used with filters.
...
def fun(s):
    ...
    try:
        username, web_ext = s.split('@')
        web, ext = web_ext.split('.')
    except ValueError as e:
        return False

    return all([username.replace('-', '').replace('_', '').isalnum(),
                web.isalnum(), len(ext) <= 3])
