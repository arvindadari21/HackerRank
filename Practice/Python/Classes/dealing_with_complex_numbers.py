...
result of their addition, subtraction, multiplication, division and modulus
operations.
...
Output Format:
    ...
    * mod(D)
    ...
    For complex numbers where the real part is zero and the complex part(B) is
...
Concept
    Python is a fully object-oriented language like C++, Java, etc. For reading
    about classes, refer here.

    Methods with a double underscore before and after their name are considered
    as built-in methods. They are used by interpreters and are generally used in
    the implementation of overloaded operators or other built-in functionality.
...
class Complex(object):
    ...
    def __truediv__(self, no):
        ...
        return Complex(
            (self.real * no.real + self.imaginary * no.imaginary) / mag,
            (self.imaginary * no.real - self.real * no.imaginary) / mag)
    ...
    def __str__(self):
        if self.imaginary == 0:
            ...
            if self.imaginary >= 0:
                ...
                result = "0.00-%.2fi".format(abs(self.imaginary))
        elif self.imaginary > 0:
            result = "%.2f+%.2fi".format(self.real, self.imaginary)
        else:
            result = "%.2f-%.2fi".format(self.real, abs(self.imaginary))
        return result
