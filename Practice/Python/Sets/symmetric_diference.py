...
Concept:
    If the inputs are given on one line separated by a space character, use
    split() to get the separate values in the form of a list:
...
if __name__ == '__main__':
    M = int(input())
    M_data = list(map(int, input().strip().split()))

    N = int(input())
    N_data = list(map(int, input().strip().split()))

    print('\n'.join(str(i) for i in sorted(
        set(M_data).union(N_data) - set(M_data).intersection(N_data))))
