...
for each i from 1 to n. Each value should be space-padded to match the width of
...
def print_formatted(number):
    # your code goes here
    len_ = len(bin(number)[2:])
    for i in range(1, number + 1):
        print(str(i).rjust(len_), oct(i)[2:].rjust(len_),
              hex(i)[2:].upper().rjust(len_), bin(i)[2:].rjust(len_))
