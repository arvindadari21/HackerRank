...
def sock_merchant(n, ar):
    val = Counter(ar).values()
    pairs = sum((x // 2) for x in val)
    return pairs
...
if __name__ == '__main__':
    ...
    ar = list(map(int, input("Enter the space separated integers "
                             "representing the colors: ").rstrip().split()))

    result = sock_merchant(n, ar)

    print("Number of pair of socks found: ", result)
